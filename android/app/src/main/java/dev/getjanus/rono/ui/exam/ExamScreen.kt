...
import dev.getjanus.rono.core.designsystem.components.SecondaryButton
...
import dev.getjanus.rono.ui.study.OptionRow
import dev.getjanus.rono.ui.study.OptionVisual
import dev.getjanus.rono.ui.study.SessionSummaryView
...
fun ExamScreen(
    ...
    when (val phase = state.phase) {
        is ExamPhase.Loading, is ExamPhase.Submitting ->
            Box(Modifier.fillMaxSize(), contentAlignment = Alignment.Center) { dev.getjanus.rono.ui.common.LoadingState() }
        is ExamPhase.Failed ->
            dev.getjanus.rono.ui.common.ErrorState(phase.message, onRetry = viewModel::retry)
                ...
                actions = {
                    ...
                    IconButton(onClick = viewModel::toggleFlag) {
                        Icon(
                            ...
                            tint = if (isFlagged) MaterialTheme.rono.warning else MaterialTheme.colorScheme.onSurfaceVariant,
                        )
                    }
                },
            )
    ...
    LazyRow(
        ...
        items(state.paper.size) { i ->
            ...
            Surface(
                shape = CircleShape,
                color = bg,
                border = if (flagged) androidx.compose.foundation.BorderStroke(1.5.dp, MaterialTheme.rono.warning) else null,
                onClick = { onJump(i) },
            ) {
