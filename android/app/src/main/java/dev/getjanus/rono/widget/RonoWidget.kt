 ...
 * review" that deep-links into the app (rono://daily).
 */
class RonoWidget : GlanceAppWidget() {
...
private fun WidgetBody(context: Context, snapshot: WidgetSnapshot) {
    val openDaily = actionStartActivity(
        Intent(Intent.ACTION_VIEW, "rono://daily".toUri()).setPackage(context.packageName),
    )
    Column(
