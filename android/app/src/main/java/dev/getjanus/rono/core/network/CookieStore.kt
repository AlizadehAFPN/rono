...
class CookieStore @Inject constructor(
    ...
    private val prefs = context.getSharedPreferences("rono_cookies", Context.MODE_PRIVATE)
    ...
    private fun persist() {
        ...
        prefs.edit().putString(KEY, RonoJson.encodeToString(ListSerializer(StoredCookie.serializer()), stored)).apply()
    }

    private fun loadPersisted(): List<Cookie> {
        val raw = prefs.getString(KEY, null) ?: return emptyList()
        val stored = runCatching {
            RonoJson.decodeFromString(ListSerializer(StoredCookie.serializer()), raw)
